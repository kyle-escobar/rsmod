...
abstract class KotlinPlugin(private val r: PluginRepository, val world: World, val server: Server) {
    ...
    /**
     * Set the [NpcDropTableDef] for npcs with [Npc.id] of [npc]
     */
    fun drop_tables(npc: Int, def: NpcDropTableDef) {
        check(!r.npcDropTableDefs.containsKey(npc)) { "Npc drop table definition has been previous set: $npc" }
